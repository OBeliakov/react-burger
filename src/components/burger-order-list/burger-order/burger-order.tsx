...
import { TOrder, TConstructorIngredient } from "../../../utils/types/types";
import { FormattedDate } from "@ya.praktikum/react-developer-burger-ui-components";

const BurgerOrderCard = ({ order }: { order: TOrder }) => {
  const { status, createdAt, price, name, number, ingredients } = order;
  const maxLength = 5;

  const iterateIngredients = (
  ...
  const cropIngredients = (
    ingredients: TConstructorIngredient[]
  ): JSX.Element[] | null => {
    if (ingredients.length <= maxLength) {
      return iterateIngredients(ingredients);
    } else {
      ...
      const lastShownEl = leftItemsLength ? (
        <li key={ingredients.length} className={styles.image_container}>
          <div
            className={`${styles.image_overlay} text text_type_main-default`}
          >
            +{leftItemsLength}
          </div>
          <img
            src={
              ingredients[slicedIngredients.length] &&
              ingredients[slicedIngredients.length].image
            }
      ...
      return lastShownEl ? result : iterateIngredients(slicedIngredients);
    }
  };
    ...
    <div className={`${styles.card} pb-6 pt-6 pl-6 pr-6`}>
      <div className={styles.info}>
        ...
        <span className="text text_type_main-default text_color_inactive">
          Сегодня {<FormattedDate date={new Date(createdAt)} />}
        </span>
      </div>
